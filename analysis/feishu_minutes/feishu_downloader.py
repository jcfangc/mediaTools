import configparser, locale, os, re, subprocess, time, sys
...
subtitle_only = (
    True if config.get("下载设置", "是否只下载字幕文件（是/否）") == "是" else False
)
...
subtitle_params = {
    "add_speaker": (
        True if config.get("下载设置", "字幕是否包含说话人（是/否）") == "是" else False
    ),
    "add_timestamp": (
        True if config.get("下载设置", "字幕是否包含时间戳（是/否）") == "是" else False
    ),
...
class FeishuDownloader:
    def __init__(self, cookie: str):
        # 获取cookie中的bv_csrf_token
        start_pos = cookie.find("bv_csrf_token=") + len("bv_csrf_token=")
        end_pos = cookie.find(";", start_pos)
        bv_csrf_token = (
            cookie[start_pos:end_pos] if end_pos != -1 else cookie[start_pos:]
        )
    ...
    def check_minutes(self):
        ...
        # 检查记录中不存在的妙记id进行下载
        if os.path.exists(f"{FEISHU_PATH}\\downloaded_minutes.txt"):
            with open(f"{FEISHU_PATH}\\downloaded_minutes.txt", "r") as f:
                downloaded_minutes = f.readlines()
            need_download_minutes = [
                minutes
                for minutes in need_download_minutes
                if minutes["object_token"] + "\n" not in downloaded_minutes
            ]
        ...
        if need_download_minutes:
            ...
            # 将下载的妙记id写入记录
            with open(f"{FEISHU_PATH}\\downloaded_minutes.txt", "a") as f:
                for minutes in need_download_minutes:
    ...
    def download_minutes(self, minutes_list):
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            with open("links.temp", "w", encoding="utf-8") as file:
                futures = [
                    executor.submit(self.get_minutes_url, minutes)
        ...
        if not subtitle_only:
            headers_option = " ".join(
                f'--header="{k}: {v}"' for k, v in self.headers.items()
            )
            proxy_cmd = ""
        ...
        for file_name, start_time in self.meeting_time_dict.items():
            ...
            if not subtitle_only:
                os.utime(
                    f"{save_path}/{file_name}/{file_name}.mp4", (start_time, start_time)
                )
            os.utime(
                f"{save_path}/{file_name}/{file_name}.{self.subtitle_type}",
    ...
    def get_minutes_url(self, minutes):
        ...
        if minutes["object_type"] == 0:
            ...
            start_time = time.strftime(
                "%Y年%m月%d日%H时%M分", time.localtime(minutes["start_time"] / 1000)
            )
            stop_time = time.strftime(
                "%Y年%m月%d日%H时%M分", time.localtime(minutes["stop_time"] / 1000)
            )
            file_name = start_time + "至" + stop_time + file_name
        else:
            create_time = time.strftime(
                "%Y年%m月%d日%H时%M分", time.localtime(minutes["create_time"] / 1000)
            )
        ...
        # 写入字幕文件
        with open(
            f"{save_path}/{file_name}/{subtitle_name}.{self.subtitle_type}",
            "w",
            encoding="utf-8",
        ) as f:
            f.write(resp.text)
    ...
    def delete_minutes(self, num):
        ...
        for index in tqdm(need_delete_minutes, desc="删除妙记"):
            ...
            params = {
                "object_tokens": index["object_token"],
                "is_destroyed": "false",
                "language": "zh_cn",
            }
            resp = requests.post(
                url=delete_url, params=params, headers=self.headers, proxies=proxies
            )
            if resp.status_code != 200:
                raise Exception(
                    f"删除妙记 http://meetings.feishu.cn/minutes/{index['object_token']} 失败！{resp.json()}"
                )
...
if __name__ == "__main__":
    ...
    # 如果填写了管理参数，则定时查询妙记空间使用情况，超出指定额度则删除最早的指定数量的妙记
        ...
        x_csrf_token = manager_cookie[
            manager_cookie.find(" csrf_token=")
            + len(" csrf_token=") : manager_cookie.find(
                ";", manager_cookie.find(" csrf_token=")
            )
        ]
        if len(x_csrf_token) != 36:
            raise Exception(
                "manager_cookie中不包含csrf_token，请确保从请求`count?_t=`中获取！"
            )
